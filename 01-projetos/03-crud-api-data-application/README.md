...
Este projeto demonstra como construir uma **aplicação completa de dados** com backend (FastAPI), frontend (Streamlit) e banco de dados (PostgreSQL). A aplicação implementa operações CRUD (Create, Read, Update, Delete) completas para gerenciamento de produtos, utilizando Docker para orquestração de todos os serviços.
...
- **ORM (Object-Relational Mapping)**: Usar SQLAlchemy para abstrair acesso ao banco de dados
- **Validação de dados**: Implementar validação com Pydantic
- **CRUD Operations**: Criar, ler, atualizar e deletar registros
- **Docker e Docker Compose**: Orquestrar múltiplos serviços com containers
- **Frontend com Streamlit**: Criar interface web para interagir com a API
...
A BlackFriday está chegando. Você sabe como o iPhone fica mais barato? Você sabe como o vídeo game é cadastrado? Você sabia que quando abre o seu navegador, nada mais é do que o seu browser fazendo um SELECT no banco do Mercado Livre 🤯

Você precisa conhecer o CRUD.

O principal responsável por tornar isso possível é o **ORM (Object-Relational Mapping)**
...
### Frontend
- **Streamlit**: Framework para criar aplicações web com Python
...
Nosso backend é uma API REST que faz a comunicação entre o frontend e o banco de dados. Vamos detalhar cada componente:
...
- **Próximos passos**: 
  - Projeto 06 (Web Scraping) para integrar APIs externas
  - Módulo de FastAPI avançado em `05-engenharia-de-dados-e-ia/06-restAPI-fastAPI-deploy/`
  - Módulo de Deploy em `02-fundamentos-dados/02-deploy-data-apps/`
